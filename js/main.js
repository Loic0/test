...
(function(){
    ...
    function animIntro(){
        const startColor = new THREE.Color(0xffffc8);
        const endColor = new THREE.Color(0x000000);
        const tl = gsap.timeline();

        tl.to("#titre", 3, {opacity:1});
        tl.to("#c", 2, {opacity:1, ease: "power1.in"}, "-=2");
        tl.to("#titre", 2, {opacity:0});
        tl.set("#titre", {display:"none"});
        if(!isMT){
            tl.call(animTriangle);
        }
        tl.to("#intro_background", 1.5, {scaleX: 0, ease:"expo2.out"});
        tl.to(startColor, 1, {r: endColor.r, g: endColor.g, b: endColor.b, onUpdate: function(){triangle.material.color = startColor;}},"-=1.5");
        tl.from(menuTxt1,1,{opacity:0, x:"50px", cursor:"default", ease:"power2.out"}, "-=0.5");
        tl.from(menuBtn1,1,{opacity:0, x:"50px", cursor:"default", ease:"power2.out"},"-=1.5");
        tl.from(".logo", 1,{opacity:0, x:"-50px", ease: "power2.out"}, "-=1.5");
    }

    function animTriangle(){
        gsap.to(triangle.rotation, 1, {z:Math.PI*2,  ease: "power2.inOut"});
        gsap.to(triangle.position, 0.5, {z:-100, ease:"power2.out"});
        gsap.to(triangle.position, 0.5, {delay:0.5,z:0, ease:"power2.in"});
        gsap.set(triangle.rotation, {delay:1, z:0});
    }

    function onMouseMove(e) {
        if(!menuOpen){
            gsap.to(triangle.rotation, 1, {x:2.55+(e.clientY-windowHalfY)*0.001, y:2.355+(e.clientX-windowHalfX)*0.001, z:0, ease:"power2.out"})}
    }
    ///////////// MENU /////////////
    ...
    function menuOn(){
        ...
        if(!isMT){
            ...
            gsap.to(".logo", 1,{delay:0.5, color:"#ffffc8"});
            gsap.to(".menu_wrapper", 1,{delay:0.5, opacity:1});
            gsap.set("#c", {zIndex: 4});
            gsap.to("#c", {x: 300});
            gsap.set(document.body, {delay: 0.8, backgroundColor: "#000000"});
            // change color            
            ...
            gsap.to(triangle.scale, 1, {x: 0.5, y: 0.5, z: 0.5});
            gsap.to(triangle.position, 1, {x: 0, y: -100, z: 0});
    ...
    function menuOff(){
        ...
        if(!isMT){
            coef = 10;
            document.body.style.backgroundColor = "#ffffc8";
            ...
            endColor = new THREE.Color(0x000000);
            gsap.to(startColor, 1, {r: endColor.r, g: endColor.g, b: endColor.b, onUpdate: function(){triangle.material.color = startColor;}})
            gsap.to(triangle.scale, 1, {x: 1, y: 1, z: 1});
            gsap.to(triangle.position, 1, {x: 0, y: 0, z: 0});
    ...
    //
    let scale, size, startColor, endColor;
    ...
    const triangle = new THREE.LineSegments(triangleWireframe, triangleMaterial);
    triangle.rotation.set(2.55,2.355,0);
    ...
    if(!isMT){
        ...
        window.addEventListener('click', animTriangle);
        ...
        window.addEventListener('touchstart', animTriangle);
    }
